...
## ProbeJS type completions

PmmoJS ships with ProbeJS support for both the v6 and v7 (Legacy) ProbeJS APIs. Type completions for all enum values, event objects, and utility classes are provided automatically when ProbeJS generates its type definitions.

You do not need to write custom ProbeJS event listeners for PmmoJS types — they are registered by the plugin during mod initialization.

For VSCode users, the ProbeJS Legacy plugin also provides code snippets (e.g., type `@pmmo_internal` to get a scaffolded internal handler).
